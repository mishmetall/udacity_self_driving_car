...
work good with plain data without preprocessing except scaling to 0..1 range.
...
| Dropout	      	|  				|
| Convolution 3x3	    | 1x1 stride, valid padding, outputs 11x11x16		|
| RELU					|												|
| Max pooling	      	| 2x2 stride,  outputs 5x5x16 				|
| Dropout	      	|  				|
| Flatten				|												|
...
but it optimize model much slower. Batch size was chosen to be 200, 50 epochs was
used to train. Learning rate was considered to be 0.001
...
simple change to 3x3 instead of 5x5 was enough to significantly improve model's
...
All of them might be easy to classify
...
As we see visually, everything is correct

#### 3. Describe how certain the model is when predicting on each of the five new images by looking at the softmax probabilities for each prediction. Provide the top 5 softmax probabilities for each image along with the sign type of each probability. (OPTIONAL: as described in the "Stand Out Suggestions" part of the rubric, visualizations can also be provided such as bar charts)

The code for making predictions on my final model is located in the 17th cell of the Ipython notebook.

For all images network was pretty confident in its prediction, as expected. Only
for slippery road it was less confident, and we can also see that it's really not
so obvious to distinguish it from Dangerous curve to the right sign.
